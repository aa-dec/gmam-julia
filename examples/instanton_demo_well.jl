...
cfg = Params(title="Weird Well potential", prefix="well")
...
"""
    V(u::Vector{Float64})
Returns the potential V(x,y) = (x-1)^2(x+1)^2y^2(y+1)^2 - ϵ x + ϵ * x^2
Multi- well potential with minima around (-1, -1) and (1, 0)
"""
function V(u::Vector{Float64})
    x, y = u[1], u[2]
    return (x-1)^2 * (x+1)^2 * y^2 * (y+1)^2 - ϵ *x + ϵ * x^2
end

"""
    dV(u::Vector{Float64})
Returns the Gradient vector [dV/dx, dV/dy]
"""
function ∇V(u::Vector{Float64})
    x, y = u[1], u[2]
    dx = 2*y^2 * (x-1)^2 * (x+1) * (y+1)^2 + 2*y^2 * (x-1) * (x+1)^2 * (y+1)^2 -  ϵ + 2 * ϵ * x
    dy = 2 * (x-1)^2 * (x+1)^2 * y * (y+1)^2 + 2*(x-1)^2 * (x+1)^2 * y^2 * (y+1)
    
    return [dx, dy]
end

"""
    ddV(u::Vector{Float64})
...
function ∇²V(u::Vector{Float64})
    ...
    h11 = 2*y^2 * yp1_sq * (xm1_sq + 4*(x-1)*(x+1) + xp1_sq) + 2* ϵ
    h12 = 8*x*y * (2*x^2*y^2 + 3*x^2*y + x^2 - 2*y^2 - 3*y - 1)
    h22 = 2 * xm1_sq * xp1_sq * (y^2 + 4*y*(y+1) + yp1_sq)
...
"""
    dV!(u::Vector{Float64}, u::Vector{Float64})
Returns the Gradient vector [dV/dx, dV/dy]
"""
function ∇V!(dv::Vector{Float64}, u::Vector{Float64})
    x, y = u[1], u[2]
    dv[1] = 2*y^2 * (x-1)^2 * (x+1) * (y+1)^2 + 2*y^2 * (x-1) * (x+1)^2 * (y+1)^2 -  ϵ + 2 * ϵ * x
    dv[2] = 2 * (x-1)^2 * (x+1)^2 * y * (y+1)^2 + 2*(x-1)^2 * (x+1)^2 * y^2 * (y+1)
end

"""
    ddV!(u::Vector{Float64}, u::Vector{Float64})
Returns the 2x2 Hessian matrix
"""
function ∇²V!(ddv::Matrix{Float64}, u::Vector{Float64})
    ...
    ddv[1,1] = 2*y^2 * yp1_sq * (xm1_sq + 4*(x-1)*(x+1) + xp1_sq) + 2 * ϵ
    ddv[1,2] = 8*x*y * (2*x^2*y^2 + 3*x^2*y + x^2 - 2*y^2 - 3*y - 1)
    ddv[2,2] = 2 * xm1_sq * xp1_sq * (y^2 + 4*y*(y+1) + yp1_sq)
    ddv[2,1] = ddv[1,2]
end
...
# Finding stable equilibrium near (-1, -1)
